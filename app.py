...
from database import authenticate_user, register_new_session, destroy_session, is_session_valid, init_db, get_connection
from matcher import calculate_match_score, normalize_skill
from nlp_engine import extract_text_from_pdf, grade_resume
...
def _load_resume_data(user_id):
    ...
    try:
        cursor.execute("SELECT extracted_skills, resume_pdf FROM student_profiles WHERE user_id = %s", (user_id,))
