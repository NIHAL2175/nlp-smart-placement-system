...
def get_matched_skills(student_skills, job_required_skills):
    """Return the normalized skill overlap used for ATS match display."""
    student_set = set(normalize_skills(student_skills))
    job_list = normalize_skills(job_required_skills)

    return [skill for skill in job_list if skill in student_set]
...
def calculate_match_score(student_skills, job_required_skills):
    ...
    missing_skills = [skill for skill in job_list if skill not in student_set]
