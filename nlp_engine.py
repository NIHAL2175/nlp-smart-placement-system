...
def grade_resume(text):
    ...
    for sec_name, keywords in sections.items():
        if not has_section(text, keywords):
            feedback.append({
                "type": "warning",
    ...
    weak_verbs = ["helped", "worked", "did", "made"]
    found_weak = [v for v in weak_verbs if re.search(r'\b' + re.escape(v) + r'\b', text_lower)]
    ...
    if word_count < 300:
        feedback.append({
            ...
            "desc": "Your resume appears to be under 300 words, which usually reduces ATS confidence."
        })
        score -= 10
    ...
    if score == 100:
        ...
        feedback.append({
            ...
            "desc": "Your resume was processed successfully."
        })
