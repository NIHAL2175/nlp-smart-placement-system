...
from matcher import normalize_skills, get_matched_skills
...
def _normalize_extracted_skills(skills):
    cleaned = []
...
def _extract_cloud_skills(resume_text):
    ...
    prompt = (
        "Extract a comma-separated list of technical and professional skills from this text. "
        "Return ONLY the skills separated by commas, no introductory text, no bullet points. "
        f"Text: {resume_text[:2000]}"
    )
    payload = {
...
def _extract_skills_for_engine(selected_engine, resume_text):
    if selected_engine == "Deterministic Lexical Matcher (Exact Match)":
        skills = extract_skills_basic(resume_text)
        return _normalize_extracted_skills(skills)

    if selected_engine == "Contextual NER Engine (Machine Learning) ~underprocess":
        skills = custom_ner_engine.extract_skills(resume_text)
        if not skills or skills[0].startswith("Error"):
            skills = extract_skills(resume_text)
        return _normalize_extracted_skills(skills)

    if selected_engine == "Generative LLM Parser (Gemma 2B) ~For tech resume only":
        skills = experimental_llm_engine.extract_skills(resume_text)
        if not skills or skills[0].startswith("Error"):
            skills = extract_skills(resume_text)
        hybrid_skills = skills + extract_skills_basic(resume_text)
        return _normalize_extracted_skills(hybrid_skills)

    if selected_engine == "Cloud API Parser (Free Public Endpoint)":
        skills = _extract_cloud_skills(resume_text)
        if not skills:
            skills = extract_skills(resume_text)
        hybrid_skills = skills + extract_skills_basic(resume_text) + extract_skills(resume_text)
        return _normalize_extracted_skills(hybrid_skills)

    return _normalize_extracted_skills(extract_skills_basic(resume_text))


def _finalize_ats_score(selected_engine, resume_text, skills):
    default_job_skills = ["python", "sql", "react", "aws", "docker", "git", "linux", "machine learning"]
    match_score, missing_skills = calculate_match_score(skills, default_job_skills)
    matched_skills = get_matched_skills(skills, default_job_skills)
    base_score, feedback = grade_resume(resume_text)

    final_score = int((base_score * 0.7) + (match_score * 0.3))

    if selected_engine == "Deterministic Lexical Matcher (Exact Match)":
        final_score -= 5
    elif selected_engine == "Contextual NER Engine (Machine Learning) ~underprocess":
        final_score += 0
    elif selected_engine == "Generative LLM Parser (Gemma 2B) ~For tech resume only":
        final_score += 5
    elif selected_engine == "Cloud API Parser (Free Public Endpoint)":
        final_score += 8

    final_score = max(0, min(100, final_score))
    return final_score, base_score, match_score, matched_skills, missing_skills, feedback
        ...
        with prof_col1:
            ...
            if uploaded_file is not None:
                if st.button("Extract Skills & Save", type="primary", use_container_width=True):
                    ...
                    if selected_engine == "Deterministic Lexical Matcher (Exact Match)":
                        spinner_message = "🔍 Scanning against known skills..."
                    elif selected_engine == "Contextual NER Engine (Machine Learning) ~underprocess":
                        spinner_message = "🧠 Running text through Custom Trained Neural Network..."
                    elif selected_engine == "Generative LLM Parser (Gemma 2B) ~For tech resume only":
                        spinner_message = "🤖 Analyzing with Local AI... (This may take a few seconds)"
                    else:
                        spinner_message = "🌐 Routing to Blazing Fast Groq API (Llama 3.1)..."

                    with st.spinner(spinner_message):
                        extracted_skills = _extract_skills_for_engine(selected_engine, resume_text)
                    ...
                    if extracted_skills:
                        if save_student_profile(st.session_state.user_id, resume_text, extracted_skills, pdf_bytes):
                            ...
                            final_score, base_score, match_score, matched_skills, missing_skills, feedback = _finalize_ats_score(
                                selected_engine,
                                resume_text,
                                extracted_skills,
                            )

                            st.markdown(f"## 🧠 AI Resume Score: {final_score}/100")
                            st.progress(final_score / 100)

                            with st.expander("Debug Output", expanded=False):
                                st.write("Selected Model:", selected_engine)
                                st.write("Extracted Skills:", extracted_skills)

                            # Keep feedback simple: show concise warnings/info/success from grade_resume.
                        ...
                        st.error("Extraction failed: No skills found.")
